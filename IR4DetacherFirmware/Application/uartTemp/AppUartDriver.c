...
static UsartDriverBuffers AppUartDriverBuffers[NUMBER_OF_UARTS];
...
{
    return cyclic_fifo_queue_slots_used((const cyclic_fifo_queue_struct*)&(inst_uart[uartIndx].tx_queue));
}

unsigned int AppUartDriver_rx_bytes_remaining(uart_indx_t uartIndx)
{
    return cyclic_fifo_queue_slots_used(&(inst_uart[uartIndx].rx_queue));
}
...
{
	//TODO: AppUartDriver_is_transmitter_enabled() it's used by the IR code.
	(void)uartIndx;
	return 1;
}
bool AppUartDriver_is_receiver_enabled(uart_indx_t uartIndx)
{
	//TODO: Write AppUartDriver_is_receiver_enabled()
	(void)uartIndx;
	return 1;
}
...
{
	return inst_uart[uartIndx].uart_hal->uart_module->ISR & USART_ISR_BUSY;
}
