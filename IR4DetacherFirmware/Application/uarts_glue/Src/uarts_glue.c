...
static AppUartDriver_t *UartDriverPtr[NUMBER_OF_UARTS] =
{
	(void*)0,
	(void*)0,
};
...
void uart_gl_init_this_uart(uart_indx_t uartIndx)
{
	...
	UartDriverPtr[uartIndx] = AppUartDriver_init(uart_defs[uartIndx].uart_bus_num,
			                          (uart_hal_t*)&uart_defs[uartIndx].uart_hal,
					                  (uint8_t *)UsartBufs[uartIndx].rx_buf,
									  RX_BUF_SIZE,
									  (uint8_t *)UsartBufs[uartIndx].tx_buf,
									  TX_BUF_SIZE);

	if (uartIndx == IR_INTERFACE_UART_INDEX)
	{
		// disable TX on the IR interface UART
		AppUart_disable_tx(uartIndx);
	}

	// Enable RX on the UART
	AppUart_enable_rx(uartIndx);
}
...
bool uart_gl_Uart_is_inactive(uart_indx_t uartIndx)
{
	...
	{
		retVal = ((0 == AppUartDriver_tx_bytes_remaining(uartIndx)) && /* no bytes in the Tx queue                                 */
				  (0 == AppUartDriver_rx_bytes_remaining(uartIndx)) && /* no bytes in the Rx queue                                 */
				  //(!AppUartDriver_is_transmitter_enabled(uartIndx)) && /* UART transmitter disabled                                */
				  //( AppUartDriver_is_receiver_enabled(uartIndx))    && /* UART receiver is enabled (front porch delay has expired) */
				  (!AppUartDriver_is_receiving(uartIndx))           && /* UART is not actively receiving                           */
				  (!AppUartDriver_is_transmitting(uartIndx)) );                       /* UART is not actively transmitting    */
	}
